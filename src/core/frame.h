...
namespace pol {
	...
	public:
		...
		__forceinline Float CosTheta(const Vector3f& d) const {
			return d.y;
		}

		//assume vector is in local coordinate
		//return sine of the angle between vector and normal
		__forceinline Float SinTheta(const Vector3f& d) const {
		...
		__forceinline Float SinPhi(const Vector3f& d) const {
			Vector3f t = Normalize(Vector3f(d.x, 0, d.z));
			return t.z;
		}
		...
		__forceinline Float TanPhi(const Vector3f& d) const {
			Vector3f t = Normalize(Vector3f(d.x, 0, d.z));
			return t.z / t.x;
		}
		...
		__forceinline Float CosPhi2(const Vector3f& d) const {
			Vector3f t = Normalize(Vector3f(d.x, 0, d.z));
			return t.x * t.x;
		}
		...
		__forceinline Float SinPhi2(const Vector3f& d) const {
			Vector3f t = Normalize(Vector3f(d.x, 0, d.z));
			return t.z * t.z;
		}
		...
		__forceinline Float TanPhi2(const Vector3f& d) const {
			Vector3f t = Normalize(Vector3f(d.x, 0, d.z));
